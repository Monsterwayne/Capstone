...
console.log("posts: ", posts);
  ...
  {posts.length > 0 ? (
    posts.map((item, index) => {
      if (!item.publicKey || !item.account || !item.account.title) {
        // If the item does not have a publicKey, account, or title, return null or some placeholder
        return <div key={`post-placeholder-${index}`}>Loading...</div>; // Placeholder for loading state or empty posts
      }

      const explorerLink = generateExplorerLink(item.publicKey.toString());
      const pubKeyShort = truncatePubKey(item.publicKey.toString());

      
      console.log("item: ", item.account);
      console.log("item: ", item.account.content);

      return (
        <article 
          className="post__card-2"
          onClick={() => handlePostClick(item.account.txSignatures)}
          key={item.publicKey.toString()} // Ensure this key is unique for each post
        >
          <div>
            {/* Displaying the PDA of the POST here */}
            <p>Post PDA: <a href={explorerLink} target="_blank" rel="noopener noreferrer">
                {pubKeyShort}
              </a></p>
            {/* Displaying the title of the post */}
            <h3>{item.account.title}</h3>
            <p>{item.content}</p> {/* Displaying the content of the post */}
    ...
    <p>No posts available</p> // Display this when there are no posts
  )}
</div>
