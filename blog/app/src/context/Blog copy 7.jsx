  ...
  const retrievePost = async (txSignatures) => {
    ...
    for (const signature of txSignatures) {
      ...
      if (
        ...
        postContent += Buffer.from(chunkData).toString("utf-8");
      }
    }
    return postContent;
  };
    ...
    const fetchUserAndPosts = async () => {
      if (program && publicKey) {
        try {
          ...
          const postsWithContent = await Promise.all(postAccounts.map(async (postAccount) => {
            const content = await retrievePost(postAccount.account.txSignatures);
            return {
              ...postAccount.account,
              content, // Add content to each post object
            };
          }));
          console.log('PostsWithContent after fetching:', postsWithContent);
          setPosts(postsWithContent); // Update the posts state with content
        } catch (error) {
          console.log("Error fetching user or posts:", error);
        }
      }
    };
